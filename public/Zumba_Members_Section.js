        ...
        function formatPowerId(id) { return padId(id); }
        ...
        async function fetchMemberAttendanceMap(powerId, startDate, endDate) {
            ...
            try {
                await Promise.all(idsToQuery.map(async (idVariant) => {
                    const q    = query(collection(db, 'zumba_attendance'), where('powerId', '==', idVariant));
        ...
        function listenMembersRealtime() {
            ...
            const membersQuery = query(collection(db, 'zumba_members'), orderBy('powerId', 'asc'));
        ...
        function listenAttendanceRealtime() {
            ...
            const today = getToday();
            const q     = query(collection(db, 'zumba_attendance'), where('date', '==', today));
                    ...
                    <div class="member-card ${statusClass} ${highlighted}" data-id="${member.powerId}" role="button" tabindex="0">
                        <div class="member-info">
                            ...
                            <div class="member-details">
                                ...
                                <div class="member-id">
                                    <span class="id-label">Power ID:</span>
                                    <span class="id-value">${formatPowerId(member.powerId)}</span>
                                </div>
                            </div>
                        </div>
        ...
        async function showGraphModal(powerId) {
            ...
            el.graphMemberId.textContent   = `Power ID: ${formatPowerId(member.powerId)}`;
