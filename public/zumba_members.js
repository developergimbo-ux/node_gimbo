import { initializeApp }        from "https://www.gstatic.com/firebasejs/12.12.0/firebase-app.js";
  ...
  async function checkExpiredPausedMembers(members) {
    ...
    for (const m of expired) {
      try {
        await updateDoc(doc(db, 'zumba_members', m.id), { status: 'left' });
  ...
  const membersCol = collection(db, 'zumba_members');
  ...
  // Firebase stores: avg_time_spent_on_zumba_by_members
  onSnapshot(doc(db, 'zumba_settings', 'config'), (snap) => {
    const data      = snap.exists() ? snap.data() : {};
    const zumbaTiming = data.zumbaTiming || {};
    const timings   = {
      morningOpen:  zumbaTiming.morningOpen  || data.morningOpen  || null,
      morningClose: zumbaTiming.morningClose || data.morningClose || null,
      eveningOpen:  zumbaTiming.eveningOpen  || data.eveningOpen  || null,
      eveningClose: zumbaTiming.eveningClose || data.eveningClose || null,
    };
    // ✅ FIXED: correct field name matching Firebase document
    const avgHours   = parseFloat(data.avg_time_spent_on_zumba_by_members) || 2;
  ...
  onSnapshot(collection(db, 'zumba_attendance'), (snap) => {
    _cachedAttendance = [];
    snap.forEach(d => {
  ...
  window.openMemberModal = function(id = null) {
    ...
    if (id) {
      ...
      if (m) {
        ...
        document.getElementById('memberFitnessGoal').value = 'Zumba';
  ...
  window.saveMember = async function(e) {
    ...
    try {
      if (docId) {
        await updateDoc(doc(db, 'zumba_members', docId), {
        ...
        const feesQ    = query(collection(db, 'zumba_fees'), where('memberId', '==', docId));
        const feesSnap = await getDocs(feesQ);
        const feeDoc   = buildFeeDoc(docId, powerId, name, fee, joinDateStr, pkg);
        if (!feesSnap.empty) {
          await updateDoc(feesSnap.docs[0].ref, feeDoc);
        } else {
          await addDoc(collection(db, 'zumba_fees'), feeDoc);
        }
        showToast('Zumba Member updated successfully', 'success');
      } else {
        const memberRef = await addDoc(collection(db, 'zumba_members'), {
          powerId, name, phone, email,
          package: pkg, fitnessGoal, status, fee,
          membershipDays,
          joinDate:    joinDateStr,
        ...
        await addDoc(collection(db, 'zumba_fees'), buildFeeDoc(memberRef.id, powerId, name, fee, joinDateStr, pkg));
        showToast('Zumba Member added successfully', 'success');
      }
  ...
  window._pauseMember = async function(id) {
    try {
      await updateDoc(doc(db, 'zumba_members', id), { status:'paused', pausedDate: Timestamp.now() });
      showToast('Zumba Member paused', 'success');
  ...
  window._activateMember = async function(id) {
    try {
      await updateDoc(doc(db, 'zumba_members', id), { status:'active' });
      showToast('Zumba Member activated', 'success');
    } catch(err) { showToast('Error: ' + err.message, 'error'); }
  };
  ...
  window._deleteMember = async function(id) {
    ...
    try {
      await deleteDoc(doc(db, 'zumba_members', id));
      showToast('Zumba Member deleted', 'success');
    } catch(err) { showToast('Error: ' + err.message, 'error'); }
  };
  ...
  window.deleteAllMembers = async function() {
    ...
    try {
      ...
      await Promise.all(snapshot.docs.map(d => deleteDoc(doc(db, 'zumba_members', d.id))));
      showToast('All Zumba members deleted successfully', 'success');
  ...
  window.confirmRejoin = async function() {
    ...
    try {
      ...
      await updateDoc(doc(db, 'zumba_members', id), {
        status:      'active',
        joinDate:    date,
      ...
      await addDoc(collection(db, 'zumba_fees'), buildFeeDoc(id, powerId, m?.name || '', m?.fee || 0, date, pkg));

      showToast('Zumba Member rejoined successfully', 'success');
