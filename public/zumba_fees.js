...
function processAndRender() {
  ...
  // Only count fees that belong to a real member (skip sample/orphan docs)
  const knownPids = new Set(_members.map(m => String(m.powerId||'').trim()).filter(Boolean));
  _fees.forEach(f=>{
    if (!knownPids.has(String(f.powerId||'').trim())) return; // skip orphan/sample docs
    if((f.status||'').toLowerCase()!=='paid') return;
    // Use month field first (most reliable — matches Firebase doc structure)
    if (f.month && typeof f.month === 'string') {
      if (f.month.trim().substring(0,7) === currentMonth) revenue += Number(f.amount||0);
      return;
    }
    // Fallback: derive from paymentDate or date field
    const dateStr = f.paymentDate || f.date || null;
    if (!dateStr) return;
    let fd;
    if (typeof dateStr === 'string' && /^\d{4}-\d{2}-\d{2}/.test(dateStr.trim())) {
      const parts = dateStr.trim().split('-').map(Number);
      fd = new Date(parts[0], parts[1]-1, parts[2]); // local — avoids UTC shift
    } else {
      fd = parseDate(dateStr);
    }
...
window.confirmPay = async function() {
  ...
  try {
    await addDoc(collection(db, 'zumba_fees'), {
...
// ── Firebase listeners ────────────────────────────────────────
onSnapshot(collection(db,'zumba_members'), snap=>{
...
onSnapshot(collection(db,'zumba_fees'), snap=>{
